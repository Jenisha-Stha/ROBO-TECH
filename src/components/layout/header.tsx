...
const Header = () => {
  ...
  const { data: searchResults, isLoading: isSearching } = useSupabaseQuery<
    Course[]
  >({
    ...
    queryFn: async () => {
      ...
      const { data, error } = await supabase
        ...
        .eq("is_active", true)
        .or(
          `title.ilike.%${debouncedSearchTerm}%,detail.ilike.%${debouncedSearchTerm}%`,
        )
        .limit(20);
      ...
      const ranked = (data ?? []).sort((a, b) => {
        ...
        const aExact =
          a.title?.toLowerCase() === term || a.detail?.toLowerCase() === term;
        const bExact =
          b.title?.toLowerCase() === term || b.detail?.toLowerCase() === term;
        if (aExact && !bExact) return -1;
        if (bExact && !aExact) return 1;
        const aStarts =
          a.title?.toLowerCase().startsWith(term) ||
          a.detail?.toLowerCase().startsWith(term);
        const bStarts =
          b.title?.toLowerCase().startsWith(term) ||
          b.detail?.toLowerCase().startsWith(term);
      ...
      return (
        ranked.slice(0, 5).map((course) => ({
          ...course,
          lesson_count:
            course.lessons?.filter(
              (lesson) => !lesson.is_erased && lesson.is_active,
            ).length || 0,
        })) || []
      );
    },
  ...
  const playTypingSound = () => {
    if (typingSoundRef.current) {
      ...
      typingSoundRef.current
        .play()
        .catch((err) => console.debug("Audio play failed:", err));
    }
  };
  ...
  return (
    <nav className="navbar">
      ...
      <div className={`mobile-bottom-nav ${isMobileMenuOpen ? "hidden" : ""}`}>
        <Link
          to="/"
          className={`mobile-bottom-nav-link ${location.pathname === "/" ? "active" : ""}`}
        >
        ...
        <Link
          to="/about-us"
          className={`mobile-bottom-nav-link ${location.pathname === "/about-us" ? "active" : ""}`}
        >
          <Info className="mobile-bottom-nav-icon" />
          <span>About</span>
        </Link>
        <Link
          to="/our-courses"
          className={`mobile-bottom-nav-link ${location.pathname === "/our-courses" ? "active" : ""}`}
        >
          <BookOpen className="mobile-bottom-nav-icon" />
          <span>Courses</span>
        </Link>
        <Link
          to="/become-our-partner-school"
          className={`mobile-bottom-nav-link ${location.pathname === "/become-our-partner-school" ? "active" : ""}`}
        >
